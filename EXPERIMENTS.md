...
## 2026-08-31 — V68 eight-hour training ablations

Experiment: Keep FEATURE55/MODEL68, the 920,611-parameter W64/L2/H4/FF128 architecture, the immutable `target/v68-native-a0-b24/initial.pt` initialization, native 1×512 actors, batch 8,192, temperature `.8`, BF16, and a nominal `5,000,192`-decision budget. First sweep learning rates `[3e-4, 4.5e-4, 6e-4, 9e-4, 1.2e-3]`, then screen one-factor changes at the selected `9e-4`. Evaluate final checkpoints greedily on the same A0/+20 panel: 64 runs per character from seed `4,620,000,000`, 320 runs total. Entropy here is mean Shannon policy uncertainty over legal actions, in nats; its coefficient rewards less-peaked action distributions.

LR-grid results:

| LR | Stage | Held-out wins | Mean floor | Boss conversion | Caps |
|---:|---:|---:|---:|---:|---:|
| `3e-4` | +24 | 114/320 | 44.45 | 48.3% | 1 |
| `4.5e-4` | +16 | 107/320 | 46.42 | 47.6% | 60 |
| `6e-4` | +16 | 135/320 | 47.48 | 55.3% | 60 |
| `9e-4` | +16 | 148/320 | 47.95 | 59.4% | 59 |
| `1.2e-3` | +24 | 76/320 | 41.10 | 42.0% | 55 |

The one-seed grid selected `9e-4`. Both extremes were optimization failures: `3e-4` never promoted and used 54.3% of attempted rows; `1.2e-3` used about 27% and never promoted. The middle rates all reached +16, with `9e-4` producing the best held-out result.

One-factor screens at `9e-4`:

| Change | Stage | Utilization | Accepted/rejected proposals | Held-out wins | Caps |
|---|---:|---:|---:|---:|---:|
| temperature `.6` | +20 | 95.2% | 201/6 | 109/320 | 0 |
| one-pass rows | +24 | 16.3% | 82/1,292 | 75/320 | 0 |
| character-balanced rows | +24 | 8.9% | 46/1,451 | 52/320 | 0 |
| winning replay, capacity 4,096 | +24 | 13.0% | 68/1,394 | 122/320 | 0 |
| GAE lambda `.95` | +24 | 17.0% | 86/1,300 | 80/320 | 0 |
| batch 4,096 | +24 | 5.1% | 53/2,958 | 27/320 | 41 |
| entropy start `.003` | +24 | 9.4% | 49/1,436 | 29/320 | 73 |

Temperature `.6` remained stable but learned more slowly and lost 12.2 held-out win-rate points. Every other change stalled the optimizer after its early updates. Winning replay admitted native trajectories correctly and trained 86 replay rows before its replay gradient made every later proposal fail the fresh-policy KL check. One-pass sampling removed useful reuse; character balancing oversampled weak/noisy strata; lambda `.95`, batch 4,096, and lower entropy all increased effective policy-step brittleness. The last four runs used two Torch threads and default internal evaluation namespaces rather than the canonical one-thread namespaces; they are screening evidence only. They never promoted, so internal promotion panels did not affect their training stage, and their rejection margins are large enough to exclude them from confirmation.

Confirmation: Repeat `6e-4` and `9e-4` from the same immutable weights with fresh training seed `3,300,000,000` and canonical one-thread/evaluation settings. `9e-4` promoted to +20 at step `3,228,607`, finished at step `5,053,666` with 91.6% utilization, and scored 106/320 held-out wins with 65 caps. `6e-4` promoted later at step `3,962,638`, finished at step `5,000,314` with 100% utilization, and scored 131/320 with 49 caps. Across the original and confirmation seeds, `6e-4` scored `266/640` (41.6%) with 109 caps; `9e-4` scored `254/640` (39.7%) with 124 caps.

Conclusion: Use LR `6e-4` as the robust default. `9e-4` is a higher-variance option with faster promotion and the best single run, but it did not retain the aggregate lead. Keep temperature `.8`, GAE lambda `1`, batch 8,192, priority decay `3`, uniform sampling, no winning replay, and entropy `.01 -> .001`. Next test batch 16,384 at `6e-4`, or a batch-coupled grid around `[4.5e-4, 6e-4, 7.5e-4]`; address deterministic 2,048-step loops before increasing LR again.

Implementation notes: Training now exposes winning capacity, priority decay, and character-balanced sampling for controlled experiments. Native winning trajectories reconstruct their behavior policy on the learner. Rare padded-action under-allocation and multiprocessing queue shutdown hangs were fixed. Two failed replay integration attempts remain under `target/ablation-v68-winreplay*` and are excluded from model-quality evidence.

## 2026-09-01 — Native stochastic MCTS expert iteration

Implementation: Native actors can run disabled-by-default shadow MCTS at sampled combat states. Each simulation clones the root, resamples hidden combat RNG independently while preserving public information, selects with PUCT, stops after a configurable number of turns or actions, and evaluates the frontier with the existing win-value and policy heads. Terminal wins/deaths use values 1/0; there is no intermediate reward. Public-state transpositions share nodes, leaf inference is batched across roots, tree traversal is parallel, and search uses a separate RNG stream so executed actions remain exactly on-policy.

Training: Root visit counts become a configurable-temperature expert distribution. Fresh targets are transported with packed actor rows and minimize `KL(search || policy)` up to an additive target-entropy constant, weighted by `--expert-weight` and bounded by `--expert-max-lag`. Boss roots can use a separate simulation budget. Controls are `--mcts-fraction`, `--mcts-simulations`, `--mcts-boss-simulations`, `--mcts-turns` (`0` means combat end), `--mcts-max-depth`, `--mcts-prior-temperature`, `--mcts-target-temperature`, and `--mcts-exploration`.

Validation: A paired 64-environment release test preserved all sampled actions, public rows, states, seeds, and resets for 5,120 decisions while running 55,232 simulations. The clean V68 initialization completed an end-to-end 2,157-decision smoke with 1,211 searched roots, 7,124 simulations, eight accepted updates, finite weights, mean expert cross-entropy 1.721, and 2,273 actor decisions/s at 25% search with 4 ordinary/8 boss simulations. A saturated 16-simulation benchmark sustained about 9,000 incremental simulations/s on this machine.

## 2026-09-01 — Expert-iteration training pilots

Experiment: Match the original V68 `9e-4` run from `target/v68-native-a0-b24/initial.pt`: FEATURE55/MODEL68, batch 8,192, policy temperature `.8`, BF16, identical training and evaluation seeds, and a nominal 5,000,192 decisions. Search 25% of actionable combat roots and train with expert weight `.1`. Vary MCTS budget and maximum action-sequence depth.

| Search | Last decisions | Mean terminal floor | Window wins | Accepted updates | Expert KL | Actor decisions/s | Search simulations/s |
|---|---:|---:|---:|---:|---:|---:|---:|
| `16`, boss `64`, depth `64` | 1,316,455 | 42.41 | 15/95 | 140/140 | not instrumented | 608 | 6,160 |
| `8`, boss `16`, depth `32` | 792,644 | 36.38 | 2/117 | 82/82 | .036 | 1,689 | 6,383 |
| `8`, boss `16`, depth `8` | 1,311,517 | 40.93 | 8/95 | 142/142 | .051 | 1,323 | 4,993 |
| `8`, boss `16`, depth `1` | 68,484 | 20.65 | 0/183 | 7/7 | .044 | 2,529 | 9,880 |

The plotted report curves are in `target/expert-iteration-curves-20260901.png`. At matched early checkpoints, depth-8 search consistently learned faster than the no-search control: at about 0.56M decisions its mean floor was 29.14 versus 24.26; at about 0.92M it was 39.44 with 7/103 wins versus 30.97 with 1/151 wins. Search targets were genuinely different from the policy (`KL(search || policy)` typically `.03-.055`) while all expert-iteration policy updates were accepted and row utilization stayed at 100%.

Systems result: larger/deeper searches have poor tail latency when few roots remain to batch; observed calls left most Rayon workers idle for several minutes. The multi-hour gaps in the depth-8 artifact were instead workstation suspension: it resumed with zero sampler restarts and zero watchdog-dropped steps. None of the expert runs reached the full 5M target before the 08:30 wall-clock deadline, so final held-out model-quality claims remain unproven.

Conclusion: Expert iteration is promising, and `8` simulations with depth `8` is the best configuration tested. It improves early curriculum learning without reducing optimizer utilization, while `16/64` spends too much compute and depth `32+` has unacceptable sparse-root latency. The next clean confirmation should run depth `8`, ordinary/boss budgets `8/16`, fraction `.25`, and expert weight `.1` to the full 5,000,192 decisions on an awake/keep-alive worker, then evaluate on the canonical A0/+20 panel. In parallel, implement multi-leaf batching or virtual loss within each tree so sparse roots can use all Rayon workers.

## 2026-09-01 — Turn-start batched MCTS and intermediate policy targets

Implementation: Search eligibility is now the first actionable policy call of each combat turn, tracked independently per environment and reset on new runs or snapshots. In-turn actor calls do not search again. Each root uses up to 16 concurrent virtual-visit lanes per wave; lanes from all active roots fill a configurable global batch. Simulations run in parallel, identical public leaves within a wave share one model evaluation, and every RNG path is backed up separately. Search remains disabled by default; the intended experiment configuration is 128 simulations, a one-turn horizon, and maximum depth 64.

Training: Every tree node with at least `--mcts-min-visits` visits and two legal actions can emit a packed policy-only example, capped by `--mcts-max-targets` per root. These examples use a separate bounded, versioned expert buffer and never receive PPO advantages or critic targets. Stable row IDs keep pending expert batches valid while actors append or evict buffer rows. New controls are `--mcts-batch-size`, `--mcts-min-visits`, `--mcts-max-targets`, `--expert-batch`, and `--expert-capacity`; reports include eligible turn starts, inference batches, leaf batch size, targets per root, target depth/visits, and expert-buffer use.

Validation: All `181/181` Rust tests, Python compilation, the release extension build, and the full MPS probe pass. The probe proves a search preview leaves the sampled action/state/seed stream unchanged and that the immediately following call on the same turn searches zero roots. A 32-environment 128×64 benchmark searched 18 roots and completed 2,304 simulations in `0.266s`; nine inference batches evaluated 1,658 unique leaves and emitted 68 targets, including 50 intermediate nodes up to depth 2. The exact 128×64 learner smoke in `target/expert-iteration-128x64-smoke-20260901` searched all 165 eligible turn starts, ran 21,120 simulations at 5,676/s, emitted 510 targets (`3.09/root`), trained 256 expert rows with mean depth `.69` and mean visits `55.6`, accepted all four updates, and retained 100% PPO row utilization. Mean expert cross-entropy was `1.726` and `KL(search || policy)` was `.202`.

Production: Started `target/expert-v68-lr9e4-turn-start-128x64-5m` from the immutable V68 step-zero checkpoint for 5,000,192 decisions. It uses the paired canonical seeds, LR `9e-4`, batch 8,192, temperature `.8`, the `.01 -> .001` entropy schedule, search on every eligible turn start, 128 simulations, one-turn/64-action horizon, inference batch 256, minimum 16 target visits, up to 64 targets per root, expert batch 2,048, capacity 65,536, and expert weight `.1`.

## 2026-09-01 — Process-scoped glog logging

Implementation: Future training processes use Python `logging` and native Rust `tracing`. Every physical line has a glog-style severity/date/time prefix followed by fixed-minimum-width PID, native thread ID, process role, thread name, and source location fields. The filename column has a minimum width of 20 characters and is never truncated. INFO and DEBUG go to stdout, WARNING and above go to stderr, and raw writes are normalized into the same format.

Each process writes `logs/<role>.stdout`, `logs/<role>.stderr`, and their combined `logs/<role>.INFO`; sampler roles are `sampler-0`, `sampler-1`, and so on. Thread identity stays in each record because Rayon work moves between shared worker threads. Machine-readable lifecycle/report records moved to `events.jsonl`. INFO telemetry covers learner heartbeats, collection chunks, search summaries, packets, optimizer updates, checkpoints, reports, promotions, and completion; slow search/update and worker failures are warnings. `--log-level` defaults to `INFO`.

Validation: `target/logging-smoke-20260901` exercised separate learner and sampler files, Python and Rust records, periodic heartbeats, MCTS summaries, optimizer updates, checkpoints, reports, and completion. A routing probe verified INFO-only stdout, WARNING+ stderr, combined INFO capture, raw stdout/stderr normalization, and clean end-of-process flushing.

## 2026-09-01 — Mature A0/+4 expert iteration

Experiment: Stop the step-zero `target/expert-v68-lr9e4-turn-start-128x64-5m` run after its `660,855`-decision report. Start directly from the preserved A0/+4 entry at `32,763,172` decisions and train through `42,763,044`, the nearest 512-aligned target to 10M additional decisions. Use turn-start search, 128 simulations, one-turn/64-action horizon, batch size 256, minimum 16 visits, expert weight `.1`, and the original constant LR `9e-4`. An initially launched no-search reconstruction toward 40M was interrupted when this direct continuation was selected; its partial artifacts remain in `target/v68-a0-b4-precollapse-40m-replay`.

Target-consumption correction: The stopped run generated `620,342` expert rows but trained only `139,264`; `434,444` became stale and `19,924` were capacity-evicted. Search produced about 9,100 targets per optimizer update while `--expert-batch 2048` consumed only 2,048, and `--expert-max-lag 4` expired the accumulating backlog after one four-update publication interval. The mature run therefore uses expert batch 8,192 and maximum lag 16. Expert cross-entropy is mean-normalized, so the nominal expert-loss weight remains `.1`; the larger batch increases target coverage rather than multiplying the loss coefficient.

Future expert-iteration runs default the expert batch to the PPO batch size and use a 131,072-row expert buffer. Explicit command-line values still override both defaults.

KL ablation: The `target-kl=.004` run was interrupted after its `692,600`-additional-decision report, with 59/70 policy attempts accepted, nine post-step discards, one pre-step rejection, and 101 backtracking retries. Restart from the same A0/+4 checkpoint with the otherwise identical 10M configuration and `--target-kl 1`, effectively disabling the pre/post optimizer KL gate while retaining the per-row `--max-log-ratio .5` freshness screen.

Artifacts: The constrained run is `target/expert-v68-a0-b4-from32763172-turn-start-128x64-10m`; the active KL-1 run is `target/expert-v68-a0-b4-from32763172-turn-start-128x64-kl1-10m`.

## 2026-09-01 — MCTS expectimax and value-geometry diagnostic

Implementation: `Game::step_with_rng` reports whether a transition advanced any named simulator RNG. `Batch.search_diagnostics` runs the production MCTS while retaining representative public nodes, then enumerates every legal continuation to the same one-turn/64-action frontier. Sixteen independently resampled hidden-state particles are propagated through the exhaustive action tree and grouped by public successor, so the reference also detects outcomes that depend on an already-random hidden deck without consuming fresh RNG. The diagnostic records every MCTS node, visit count, backed-up estimate, repeated-exhaustive estimate, direct RNG transition, stochastic public split, and exhaustive leaf value/player HP/enemy HP. `train.py search-diagnostics` samples real policy turn starts and writes raw JSON plus interactive 2D and 3D Plotly scatters.

Result: Four A0/+4 roots from the active mature checkpoint produced 348 MCTS nodes and 10,002 exhaustive public leaves. MCTS and repeated-exhaustive node values correlated `.955`, with MAE `.00170`; visit count did not predict lower error (`corr(log(1+visits), |error|) = -.039`). The visit-count target selected the policy-greedy action on all four roots and missed the exhaustive optimum on three. Its mean exact gap was `.000411`; choosing the largest sampled edge mean reduced that to `.000185`, showing substantial prior/visit-allocation inertia at 128 simulations. The acting policy's expected exact gap was only `.000464`, so the value model itself sees little available one-turn improvement.

Value geometry: Within-root linear fits assign strong positive value to player HP. Enemy remaining HP has almost no conditional coefficient on three roots (`-5.8e-5`, `+0.5e-6`, `-5.1e-6` value per HP) and matters strongly only in the near-kill root (`-6.2e-4`). This explains much of the unexpectedly small expert advantage: away from immediate combat completion, the critic is nearly indifferent to damaging the enemy, while MCTS visit targets remain anchored to the policy prior.

Artifacts: `target/search-diagnostics-v68-kl1-split.html` contains separate node and leaf-pair plots for each root; `target/search-diagnostics-v68-kl1-split.json` preserves all nodes and leaves.

Prior-temperature ablation: On immutable checkpoint `33,550,014`, flattening the MCTS prior from temperature `1` to `2` increased expanded nodes from 298 to 319, improved node-value MAE from `.00304` to `.00274`, correlation from `.9905` to `.9917`, and mean exact gap of the maximum sampled-Q action from `.000638` to `.000484`. The visit-count target improved only slightly (`.000910 -> .000875`) and remained strongly influenced by the original policy. Matched split plots are `target/search-diagnostics-v68-step33550014-prior-temp{1,2}-split.html`.

Progress critic: A durable copy of checkpoint `34,080,607` was evaluated with the same roots, 128 simulations, prior temperature `2`, and 16 hidden-state particles. The final-floor head has node correlation `.967` and RMSE `.734` floors versus `.977` and normalized RMSE `.225` for the matched win head, so its global MCTS approximation is not clearly better. It is substantially more coherent at combat completion: at root 1 with unchanged player HP, completed-combat leaves predict 25.90 terminal floors versus 25.14 with an enemy still alive; at root 2 the corresponding values are 20.65 versus 18.07. The matched plots are `target/search-diagnostics-v68-step34080607-{progress,win}-prior-temp2-split.html`, and the preserved checkpoint is `target/search-diagnostics-v68-step34080607.pt`.

Leaf-state audit: After conditioning on player and total enemy HP, the remaining progress-value variance is mostly explained by public next-turn RNG outcomes. Enemy intent explains 85% of the residual in root 0; hand contents explain 99% in root 1; hand contents plus enemy intent explain 99.3% in root 2 and 90.7% in root 3. At root 2 with player HP 67 and enemy HP 3, the low state predicts floor 17.43 with two Defends in hand, Boost Away still in the draw pile, and Sludge Spinner move 2; the high state predicts floor 18.64 with Boost Away and three Strikes in hand and move 1. Powers, energy, block, discard, exhaust, and legal-action count were constant within these HP strata. Detailed public leaf states are in `target/search-diagnostics-v68-step34080607-progress-state-details-all.json`.

Progress-prior-temperature ablation: Fresh matched searches on the same four roots compared prior temperatures `1`, `2`, `3`, and `10`. Expanded node counts were `317`, `296`, `318`, and `312`; node MAE was `.541`, `.535`, `.536`, and `.505` terminal floors, with correlations `.9673`, `.9672`, `.9697`, and `.9735`. Temperature `10` therefore improved global value approximation, but its visit-selected root-action gap rose to `.157` floors versus `.031` at temperature `1` and `.034` at temperature `3`; temperature `2` was an unstable outlier at `.253`. Maximum sampled-Q gaps were `.031`, `.157`, `.034`, and `.034` floors. With only four roots, temperatures `1` and `3` currently give the best finite-budget visit targets, while stronger flattening helps exploration without reliably allocating the 128 root visits. The combined interactive plot is `target/search-diagnostics-v68-step34080607-progress-prior-temp1-2-3-10-comparison.html`.

## 2026-09-01 — Expectimax-Q expert targets

Correction: Visit-count targets mostly reproduced the current tempered prior. Across the four diagnostic roots, total-variation distance between normalized visits and the search prior was only `.133`, `.103`, `.083`, and `.088` at prior temperatures `1`, `2`, `3`, and `10`. The visit-based run was stopped at its preserved `34,863,850`-decision checkpoint.

Implementation: Search now records sampled immediate successor frequencies for every action. Nodes are keyed by public-state digest and depth, making the finite-horizon graph acyclic. After sampling, a bottom-up expectimax pass averages successor values over sampled RNG outcomes and takes the maximum over player actions. The floor-reached critic supplies frontier values. Expert distributions are a prior-free softmax over these backed-up Q values; visit counts only gate sufficiently sampled nodes. `--mcts-q-temperature` replaces the visit-count target temperature and defaults to `.002` in normalized floor-value units.

Diagnostics: Eight matched roots with 128 simulations and 16 exhaustive hidden-state particles compared prior temperatures `1`, `3`, and `10`. Their maximum-Q action gaps were `.0142`, `.0010`, and `.0825` terminal floors. Across every eligible intermediate node, the corresponding approximate-Q argmax gaps were `.034`, `.044`, and `.050` floors, compared with current-policy expected gaps `.119`, `.138`, and `.140`. Prior temperature `3` is selected for the first training test because it gives the strongest root action while retaining broad exploration. At Q temperature `.002`, the mean target entropy is about `.62` nats and the exact expected gap remains well below the current policy. The combined node/action plot is `target/q-expectimax-progress-prior-temp1-3-10-roots8-comparison.html`.

Gradient calibration: A two-update smoke at expert weight `.1` measured weighted expert policy-head gradients `68×` PPO on average. The Q target is therefore not signal-starved; the production comparison uses expert weight `.002`, which put the two head-level gradients near the same scale in its first report (`1.02×`). The first process stopped at preserved step `33,288,375` after a development-time sampler restart loaded a newer argument schema. It also exposed that prefetch could select expert IDs before the current batch retired them. Backward-compatible sampler arguments and variable target tuples now prevent hot-reload failures, and expert batches are reserved only after the preceding batch is consumed. A startup-only clean process was then superseded before its first trajectory by the lazy-packing optimization. The next process was paused on request after three reports and 109,423 additional accepted decisions; its artifacts remain in `target/q-expert-v68-a0-b4-from32763172-prior3-q2e3-5m-fast`, but it had not reached its first periodic checkpoint. After all 181 Rust tests and the full MPS parity probe passed again, the clean five-million-decision comparison restarted from the original `32,763,172` checkpoint in `target/q-expert-v68-a0-b4-from32763172-prior3-q2e3-5m-complete-20260903`.

Restart progress: The clean run reached its first durable checkpoint at `33,027,204`. Through 268,026 additional accepted decisions, all 26 optimizer steps were accepted, row utilization was 100%, and the latest stochastic window had 7/71 wins with mean terminal floor 33.83. Search generated 246,780 targets and trained 212,992; 19,155 startup-backlog targets expired by the 16-version lag limit, none were capacity-evicted, and the live buffer had contracted to 14,633 rows. Later-state search throughput was 2.44k simulations/s and 120 actor decisions/s, versus 4.17k and 202 in the first report; simulation and feature encoding accounted for about 49% and 23% of MCTS time, while neural inference remained only 1.4%.

At the first matched 500k cut, Q had slightly higher mean terminal floor (`29.286` versus `29.027`) and more boss entries (`188` versus `183`), but fewer wins (`44/1,500`, 2.93%, versus `49/1,532`, 3.20%) and lower observed boss conversion (`23.4%` versus `26.8%`). This is an early stochastic comparison, not a stopping result. The expert startup backlog had fully drained by update 40: 337,109 targets seen, 317,717 trained, exactly 19,392 stale, zero evicted, and zero buffered; no new staleness appeared after the initial burst.

Stopping result: The clean Q run was stopped on request after 826,867 additional accepted decisions and 84/84 accepted optimizer updates. Across its first 25 matched report windows, Q produced 79/2,409 wins and mean floor 30.06 versus 86/2,421 wins and floor 29.99 for the original no-search continuation. No curriculum promotion occurred. The evidence supports no current improvement: Q slightly increased floor connectivity but did not improve wins. The latest resumable artifact is checkpoint `33,551,935`; later report-only progress remains preserved. The five-million continuation and its queued follow-ons were cancelled, and the experiment moves to equal one-million-decision control and critic/head screens.

## 2026-09-01 — Critic follow-up instrumentation

Implementation: Future runs can add a symmetric one-step consistency loss between each PPO state and its realized successor with `--critic-consistency-weight`; the next-state batch is capped by `--critic-consistency-batch`. The win target includes the immediate terminal reward, while the floor target uses the existing floor increment and progress discount. Both sides retain gradients. Reports separate win/floor consistency losses and compare the weighted consistency-head gradient with the supervised critic-head gradient. The feature defaults to zero and does not affect the active Q run.

An optional `--blended-critic` trains the progress head as the sole acting/search critic against `α·win + (1−α)·terminal_floor/52`, where `α = EMA(win rate)^0.4`; the EMA decay and exponent are configurable and checkpointed. It requires complete, undiscounted trajectories. `--head-learning-rate-multiplier` independently scales the policy and critic head parameter group while retaining migrated Adam state. Search now times tree simulation, state encoding, neural inference, and backup separately.

Search consistency: With `--search-consistency-weight`, each expert node additionally transports its sampled successor distribution. The target is the current-policy expectation over actions and the empirical expectation over independently resampled RNG successors. Unvisited policy mass is assigned to the parent value itself, so sparse coverage only weakens the constraint rather than fabricating a target. Terminal mass contributes its exact terminal floor value. Parent and child predictions both retain gradients; expectimax-max values are never used in this critic loss. A combined smoke exercised ordinary consistency, search consistency, the blended critic, and a `2×` head LR for eight accepted updates, including the row-replacement path.

Loss-scale calibration: Weight `.01` made both consistency gradients negligible. A separate-critic smoke at weight `10` for each term completed eight accepted updates with exact expert consumption (`1,916` seen and used). In its final window, ordinary-transition consistency was `9.6%` and search consistency `4.7%` of the supervised critic-head gradient; the whole-run means were `3.2%` and `3.8%`. Weight `10` is therefore the first meaningful ablation value, not `.01`.

Gradient diagnostics now also report the cosine between the weighted expert and PPO policy-head gradients. Their norm ratio measures signal scale but cannot distinguish reinforcement from conflict; the cosine will show whether expectimax-Q supervision points with or against the on-policy update in each future screen.

Search throughput: Timing showed that neural inference was only about 1% of MCTS wall time; simulation, feature encoding, and tree update were the real costs. Search therefore now uses the existing fast hasher for transpositions, compact vectors for the small stochastic successor sets, and lazy packed-row construction. Previously every one of roughly 52k expanded nodes was compressed even though only 1,872 met the training-target threshold. On the matched 52,224-simulation benchmark, lazy packing improved throughput from about 4.75k to 6.64k simulations/s under the same concurrent load. The first full report improved from 4.44k to 6.38k simulations/s and from 217 to 307 actor decisions/s, both about 43%, while reproducing the exact same 203 terminal trajectories and 17.867 mean floor before policy updates diverged.

Search-consistency packing no longer disables that optimization. Once a parent becomes eligible, its subsequently sampled successor rows are packed on demand; successor mass not represented by a packed row is conservatively assigned to the parent self-term. This preserves the current-policy/RNG expectation for represented transitions and weakens the loss where coverage is incomplete instead of fabricating a value target. Both the legacy four-field target API and the eight-field consistency target API, their probability bounds, Rust compilation with the Python feature, and the full MPS probe pass. A matched throughput/coverage benchmark remains part of the search-consistency screen.

The paired native benchmark used identical eight-environment trajectories for 96 policy calls, 125 searched roots, 16,000 simulations, 10,641 nodes, and 563 expert targets. Enabling consistency transport preserved every sampled action and every Q target, covered 99.999996% of the current-policy/RNG mass, transported 12.43 child rows per target, and added 9.0% MCTS time (`6.756s -> 7.363s`) under the same concurrent system load. This clears the implementation-level tractability gate; end-to-end learner overhead will be measured in the training screen.

Calibration: `critic-diagnostics` samples held-out current-policy trajectories and reports win/floor bias, MAE, RMSE, and correlation overall and by character, phase, current floor, and reward-screen state. A 2-run-per-character smoke on the immutable A0/+4 checkpoint resolved eight episodes and 2,613 states. It is too small for model selection, but it exposed the expected failure mode: the floor critic overpredicted eventual floor by 12.21 floors overall, with reward-screen MAE 13.99 floors versus 13.10 elsewhere. The smoke artifact is `target/critic-diagnostics-v68-step32763172-smoke.json`; a larger matched panel is required before choosing a consistency weight.

Execution order: Finish the uninterrupted 5,000,192-decision Q-only comparison and its reproducible no-search control, then evaluate both in FP32 on 64 fixed A0/+4 runs per character from seed `4,620,000,000`, matching the earlier canonical evaluator. Next, screen ordinary consistency weight `10`, search consistency weight `10`, their combination, the blended critic with `alpha = EMA(win rate)^0.4`, and a `2x` policy/value-head learning rate from the same `32,763,172` checkpoint and identical seeds. Each screen uses exactly 1,048,576 nominal decisions and receives the same held-out evaluation; guarded sequential launchers stop the chain if an upstream checkpoint or evaluation is incomplete. Promote the strongest two configurations to full 5,000,192-decision confirmations, run the larger phase-stratified critic calibration panel on the baseline and finalists, then replicate the clean control and finalists with one fresh common seed set. Selection uses held-out terminal floor and wins first, then boss entry/conversion, optimizer utilization, target coverage, gradient balance, KL behavior, and sampling throughput.

## 2026-09-03 — MODEL69 categorical terminal-progress critic

Implementation: Replace the scalar win and terminal-floor heads with one 54-logit critic. Categories `0..52` are canonical progress and category `53` is victory. Act 1 uses its run floor; ordinary Act 2 maps to `15 + floor + [floor >= 7] + [floor >= 10]`; Golden Compass Act 2 maps to `15 + floor`; Act 3 maps to `34 + floor`. A loss is labelled with the maximum canonical progress on its complete trajectory and a win with category 53.

Each actor transports its detached 54-way distribution. Critic targets are built backward as `q[T-1] = one_hot(Z)` and `q[t] = (1-lambda) p(s[t+1]) + lambda q[t+1]`; policy GAE remains exactly one and uses the normalized expected category. Critic cross-entropy rows are balanced from EMA character, phase, and canonical-floor marginals with the geometric mean of inverse-square-root frequencies, clipping to `[.25, 4]` before minibatch mean normalization. Reports preserve pre-weight marginal/target counts, post-weight per-category loss mass, and ESS. The critic has its own LR multiplier while trunk and policy-head rates stay independently fixed.

Migration: MODEL69 keeps FEATURE55 and copies the complete V68 trunk and policy plus the old progress critic's hidden layer. The 54-way output starts at zero logits, hence a uniform distribution; old Adam state is intentionally not restored across the changed head topology. Native export, MCTS, exact-search diagnostics, and final win calibration derive win probability and normalized expected category from this one distribution. Decision-budget admission now trims only after targets have been computed from complete trajectories, so requested experiment lengths are exact.

Validation: The full Python/MPS probe passes, including Python/Rust categorical-probability parity and native expectimax-Q search. All 182 Rust tests pass. An exact 4,096-decision A0/+4 smoke from the V68 step-32,763,172 checkpoint completed seven optimizer updates and stopped exactly at 32,767,268; critic loss stayed finite, balance ESS was 93.3% of rows, and the checkpoint restored all three optimizer groups plus critic-frequency EMA state.

Planned matched screen: From the same immutable V68 step-32,763,172 checkpoint and identical sampling/model seeds, train exactly 262,144 decisions at LR `9e-4`, batch 8,192, policy temperature `.8`, and no search. Sweep critic lambda `1`, `.99`, `.97` at critic LR multiplier `1`; select lambda using fixed held-out outcomes and critic calibration, then sweep critic multipliers `1`, `2`, `4`. Rerun the matched prior-temperature-3, Q-temperature-`.002`, 128-simulation expectimax-Q configuration with the selected categorical expected value.

Matched screen results: Lambda `1` and `.99` completed exactly 262,144 decisions; lambda `.97` was stopped on request after 232,212 because its curve showed no improvement. Their last reported critic losses were respectively `2.529`, `3.218`, and `3.927`, so lambda `1` was selected. At lambda `1`, critic LR multipliers `1x`, `2x`, and `4x` completed exactly 262,144 decisions with last reported losses `2.529`, `1.658`, and `1.605`; critic balancing retained ESS fractions `.955`, `.950`, and `.952`. All optimizer proposals were accepted and row utilization was 100%.

The fixed held-out A0/+4 screen used the first 16 seeds per character from seed 4,620,000,000. The unchanged source scored `6/80` wins, mean floor `33.963`, 26 boss entries, and 23.1% boss conversion. Categorical no-search results were `6/80`, `33.163`, 21, and 28.6% at `1x`; `3/80`, `35.913`, 25, and 12.0% at `2x`; and `7/80`, `34.075`, 26, and 26.9% at `4x`. Every no-search categorical configuration had zero wins for three characters. The `4x` result is the least-bad screening configuration, not evidence of improvement over baseline.

Matched expectimax-Q: `target/categorical-v69-q-lambda1-lr4x-262144-20260904` completed exactly 262,144 decisions with 57,066 searched roots, 7,304,448 simulations, and 244,853 expert targets. It trained 226,175 targets, expired 18,678, evicted none, and accepted all 28 optimizer updates. Search sustained 5,635 simulations/s and 276 actor decisions/s. On the same held-out panel it scored `11/80` wins by character `[1, 4, 0, 2, 4]`, mean floor `35.913`, 28 boss entries, and 39.3% boss conversion. Relative to the source on paired seeds it gained seven wins, lost two, and raised mean floor by `1.95`; the exact two-sided paired win test is not significant (`p=.180`). This is a promising directional result, but the panel is too small and one character remains winless, so it does not establish a better model.

## 2026-09-04 — Frozen-policy critic refit and expectimax retest

Implementation: MODEL70 keeps FEATURE55 and the architecture but expands the categorical critic to 83 logits. Act transitions now have explicit ten-point rewards: Act 1 boss `15` → Act 2 floor zero `25`, Act 2 boss `44` → Act 3 floor zero `54`, and final floor `72` → win `82`. MODEL69 checkpoints preserve the complete encoder and policy and migrate compatible critic weights into the new geometry. No uncertainty estimate or ordinal loss was added.

Depth is now a hard search constraint. A branch that reaches `--mcts-max-depth` before the requested turn/combat horizon is invalid in both sampled MCTS and repeated-exhaustive search; it is neither evaluated by the critic nor allowed into an expectimax target. Invalid MCTS edges are excluded from selection, backup, targets, and search consistency. This removes the earlier profitable action-cycle/cutoff exploit.

Critic training: `--critic-only` freezes every encoder and policy tensor and updates only the two critic layers. It retains every rollout state, including forced-action nodes, with complete-trajectory Monte Carlo categorical labels. Search nodes train through the existing symmetric one-step Bellman constraint under the frozen policy and sampled RNG expectation; max-backed expectimax values are not critic targets. The run used LR `1e-3`, batch 4,096, search-consistency weight `10`, 25% of ordinary turn starts and every boss turn, 128 simulations, prior temperature `3`, one-turn/64-action horizon, and exactly 524,288 newly collected states.

Artifact: `target/critic-v70-frozen-search-524288-20260904/latest.pt`. It consumed 557,892 rollout examples and 130,860/162,792 search targets over 138 critic-only updates; 31,932 search targets aged out, no rollout row was dropped, and a tensor audit confirmed every non-critic parameter is bit-identical to the source. The final training-window CE was `1.911` after plateauing around `1.8–2.2`.

Held-out calibration: On the same 80 frozen-policy trajectories and 31,076 states, categorical NLL improved `3.579 → 2.070`, Brier `1.024 → .786`, floor RMSE `20.79 → 14.65`, and floor correlation `.474 → .589`. Win-probability RMSE improved `.419 → .285`, correlation `.231 → .300`, and mean prediction fell from the badly overconfident `.378` to `.194` against a `.056` empirical rate. Artifacts are `target/critic-v70-frozen-{baseline,final}-diagnostics.json`.

Policy test: On the fixed 16-seed-per-character A0/+4 panel, the unchanged greedy policy scored `10/80`, mean floor `35.46`, 29 boss entries, and 34.5% boss conversion. Training-style 128-simulation expectimax scored `3/80`, floor `31.58`, 14 boss entries, and 21.4% conversion; on paired seeds it rescued one win, lost eight (`p=.0391`), and lost 3.89 floors on average. The HP heuristic `player_hp/max_hp - .1 * enemy_hp/enemy_max_hp`, evaluated by the same expectimax search, also scored `3/80`, floor `34.64`, 22 boss entries, and 13.6% conversion; it rescued two wins and lost nine (`p=.0654`) with a -.825-floor paired delta. Both modes had zero caps. Artifacts are `target/critic-v70-search-policy-sampled-16x5-20260904.json` and `target/critic-v70-heuristic-search-16x5-20260904.json`.

Exact-search cost: Correctly rejecting depth-overflow branches makes the repeated-exhaustive policy evaluator dramatically more expensive because cyclic lines must be disproven rather than scored at the cutoff. The 100k-state, 16-particle panel and a reduced 20k-state diagnostic were stopped after failing to complete even the first character in practical time; no partial result was represented as an evaluation. The sampled search comparison remains fully matched and complete.

Conclusion: The critic is substantially better calibrated globally, but maximizing its local one-turn estimates still produces a significantly worse policy. MCTS approximation is not the only problem: the simple HP heuristic also fails, and learned search covers only `47.3%` of nontrivial combat decisions (`8,398` hits, `9,357` fallbacks). The next critic work should target counterfactual action ranking directly—paired successor outcomes or terminal continuations from search states—rather than further reducing on-policy marginal NLL alone.

## 2026-09-04 — Critic RMSE decomposition and TD ablation

The diagnostic now uses native batched inference, reports target/prediction standard deviations, and assigns wins their trained category `82` rather than floor `72`. On a matched 80-run, 31,613-state panel, the frozen Monte Carlo critic has terminal-category RMSE `15.651`, MAE `12.259`, correlation `.582`, and bias `+1.087` floors.

A frozen-policy TD(0.1) continuation collected exactly 1,048,576 more states from 2,871 complete trajectories at LR `3e-4`, batch 8,192, without search. It changed RMSE only `15.651 → 15.629`, MAE `12.259 → 12.223`, correlation `.5822 → .5817`, and bias `+1.087 → +.661`; this is not a material improvement. The resumable artifact is `target/critic-v70-frozen-td01-1048576-20260904/latest.pt` and the matched diagnostic is `target/critic-v70-frozen-td01-diagnostics.json`.

Single-trajectory RMSE cannot approach two floors. A held-out panel cloned 40 live public states into 64 independently resampled continuations each. Even an oracle predicting the exact conditional mean has estimated single-outcome RMSE `14.790`; the Monte Carlo standard error of each 64-particle target is `1.849` floors. Against those continuation means, the Monte Carlo critic has observed/noise-corrected RMSE `10.536/10.372`; TD(0.1) improves this only to `10.145/9.975`. Therefore the actionable target is noise-corrected RMSE against repeated-continuation means, while single-outcome RMSE remains a dispersion metric. Artifacts are `target/critic-v70-repeated-continuation-{roots,diagnostics}-20260904.{pt,json}` and `target/critic-v70-terminal-floor-rmse-vs-current-floor-20260904.png`.

Repeated-target refit: A separate panel sampled 320 public roots across all characters and several points along each run, with 16 independently resampled terminal continuations per root. A trajectory-grouped 240/80 train/validation split prevented later states from the same source trajectory crossing the split. Frozen-head categorical cross-entropy fits at LR `1e-4`, `3e-4`, and `1e-3` selected only 20, 10, and zero optimizer steps respectively before validation degraded. The selected `1e-4` fit improved independent 40-root expected-floor RMSE from `10.536` to `9.325` (`9.139` noise-corrected), but remains far from two. This shows that repeated soft targets help, while 320 roots are far too small and the current head quickly overfits. Artifacts are `target/critic-v70-repeated-targets-train-20260904.pt`, `target/critic-v70-repeated-targets-refit-20260904.{pt,json}`.

Transition consistency: On the matched 31,613-transition panel, realized one-step expected-floor residuals have RMSE/MAE/bias `1.379/.631/+.011` floors for the Monte Carlo critic, `1.100/.455/+.010` for TD(0.1), and `1.360/.626/+.004` for the repeated-target refit. Their 95th-percentile absolute residuals are `2.595`, `1.715`, and `2.663` floors. These sampled residuals include legitimate variation from sampled actions and RNG, so they upper-bound Bellman inconsistency. TD improves local smoothness without materially improving terminal calibration, demonstrating that a critic can be locally self-consistent but globally wrong.

## 2026-09-04 — Full-combat MCTS evaluation

Implementation: Setting `mcts_turns=0` now performs batched current-policy rollouts from every newly expanded frontier node until combat completion. Normal exits use the learned critic at the post-combat state; the diagnostic heuristic uses `combat_win + .1 * remaining_hp/max_hp`, with death worth zero. Rollouts use independent hidden-state and action RNG, and branches exceeding the total action-depth limit remain invalid. Search reports rollout steps, completions, depth failures, and time. Evaluation modes `combat` and `combat-heuristic` apply a fresh full-combat search at each turn start and use sufficiently visited intermediate targets during that turn.

Configuration: The main comparison used the frozen V70 policy, 128 simulations per turn, prior temperature `3`, policy temperature `.8`, Q temperature `.002`, minimum four visits, maximum 128 targets, and maximum depth 256. Across 160 matched A0/+4 seeds, no episode was capped and only 12 of more than 1.6 million simulations exceeded the depth limit, so depth 256 is sufficient.

Results: On the first 40 roots, the original learned critic search scored `1/40` wins and mean terminal floor `26.575`, versus policy `5/40` and `33.025`; its paired floor delta was `-6.45` with 95% bootstrap interval `[-9.90,-2.90]`. The repeated-target refit improved this to `2/40` and `30.175`, but remained below policy by `2.85` floors. Better marginal critic calibration therefore does not repair counterfactual action ranking.

The survival-plus-HP search was independently confirmed. The exploratory 80 seeds scored `14/80` wins and floor `43.338`, versus policy `10/80` and `35.463`. A fresh 80-seed panel scored `13/80` and `42.225`, versus `8/80` and `33.900`; its paired floor gain was `+8.325`, 95% interval `[+6.013,+10.638]`, with 43 improvements, 33 ties, and four regressions. It rescued ten wins and lost five, but the exact paired win test remained inconclusive (`p=.302`). Pooled only as secondary context, HP search scored `27/160` versus `18/160`, with `+8.10` floors and 21 rescued versus 12 lost wins (`p=.163`). Boss conversion remained essentially unchanged because search primarily increased boss entry.

Cost: The 160 HP-search runs performed 1,630,720 simulations and about 20.5 million rollout actions with essentially no depth censoring. They were about 124–145 times slower wall-clock than policy evaluation because a fresh tree was built every turn. A ten-run 512-simulation check reached floor `41.1` with no wins, versus `34.4` and one win for those same seeds at 128 simulations; this is too small to establish a quality gain and costs roughly four times more. Whole-combat HP search is therefore a strong positive control for the search implementation and a clearly better floor-reaching policy, but not yet a statistically established run-winning policy. Practical training requires subtree reuse or a cheaper search schedule; learned-critic search requires counterfactual/post-combat ranking improvement first.

## 2026-09-04 — Full-combat expert iteration from scratch

Implementation: Full-combat rollouts now retain every visited action state instead of discarding the policy tail after frontier evaluation. Valid tails are inserted into the MCTS tree and backed up to the original root, so later simulations can revisit their intermediate nodes and branch there. Every retained node with at least two backed-up action values can emit a prior-free expectimax-Q target; the production run uses minimum visit count one and no practical per-root target cap. Depth-overflow trajectories remain invalid and contribute neither values nor targets. Fresh training can now select a curriculum entry with `--start-stage`.

Validation: All 182 Rust tests and the full Python/MPS parity probe pass. An 8,534-decision A0/+4 smoke searched 425 roots with 54,400 simulations, retained 664,433 nodes, produced 10,204 eligible expert rows, trained 7,168, and had zero stale, evicted, or depth-invalid rows. Search produced 24.0 usable multi-action targets per root and completed 99 trajectories without a cap. With expert weight `.002`, its weighted expert policy-head gradient was only `.84%` of PPO on average, so the fresh run uses `.1`; the same smoke implies an initial ratio near `.42`, while every production update continues to report the measured ratio and gradient cosine.

Run: `target/full-combat-expert-v70-a0-b4-f10-20m-ent10x` starts MODEL70 from random initialization directly at A0/+4 for exactly `20,000,256` decisions. It uses LR `6e-4`, batch 8,192, policy temperature `.8`, 128 full-combat simulations on 10% of eligible turn starts, depth 256, prior temperature `3`, Q temperature `.002`, the survival-plus-HP terminal heuristic, expert batch 8,192, capacity 131,072, maximum expert lag 16, and expert weight `.1`. Boss roots obey the same 10% sampling rate. The optimizer KL gate stays effectively disabled at threshold `1`; the corrected 10x entropy schedule is `.035 -> .0035` over the first five million A0/+4 decisions and `.0035` thereafter. The learner and sampler started successfully with independent glog stdout, stderr, and INFO files. The first report reached 38,741 accepted decisions: all four updates were accepted without retries, expert/PPO gradient norm ratio was `.551` with cosine `.923`, row utilization was 100%, 41,862 targets were produced and 32,768 trained with no stale or capacity drops, search selected 9.91% of eligible roots, and no rollout exceeded depth 256. Startup-average throughput was 111 accepted decisions/s and current actor throughput 207 decisions/s.

Interruption and resume: The original host-managed process was externally terminated without a Python exception or clean-stop event at 983,104 in-memory decisions. Reports through step 951,158 remain preserved, while the latest durable checkpoint is step 789,308. Through the last report, mean floor improved from 8.99 over the first 250k decisions to 12.12 over 750k–951k, with no wins yet. All 95 optimizer attempts were accepted without retries; all 574,990 expert targets had been consumed with no stale or capacity drops. Search remained at 10.03% of eligible roots and 583 simulations/s. The run resumed from the intact 789,308 checkpoint on 2026-09-05. Because continuation budgets must match the checkpoint modulo 512, the exact final target is now `20,000,060`, only 196 below the original rounded target. Reports produced after the checkpoint remain as preserved non-resumable evidence.

Resume verification: The continuation passed the lost high-water mark and wrote a new durable checkpoint at 1,051,809. At report step 1,255,742 it had completed 46/46 resumed optimizer updates, consumed all 267,161 generated expert targets with no stale or capacity drops, and maintained 100% row utilization. Mean terminal floor reached 13.57, up from 12.47 at step 1,020,946, but there were still no wins. The expert/PPO policy-head gradients were strongly opposed in the latest window (norm ratio `.646`, cosine `-.956`), so this remains the primary monitored learning-risk signal.

Second interruption and resume: The sampler watchdog recovered three wedges, then the fourth wedge at 11:10 on 2026-09-05 exhausted the per-process three-restart allowance. The learner cleanly flushed 8,138,167 accepted decisions to `latest.pt` and exited; checkpoint `8,137,378` has the logged SHA-256 `4bb1b84e76c433cd2d65d1ed44016578f60e62d021db185db40ea7da307897da`. No optimizer proposal had been rejected, and no expert row had been stale or capacity-evicted. The run resumed from the final `latest.pt` state at 17:19 with every model, optimizer, search, loss, and seed setting unchanged. Since `20,000,060 - 8,138,167` is not divisible by 512, this main continuation targets 19,999,671; one final 389-environment sampling step will then reach exactly 20,000,060. The resumed sampler emitted its first accepted packet successfully and started a fresh watchdog allowance.

Third interruption and resume: The resumed process recovered three more sampler wedges, then generation 3 wedged at 01:05 on 2026-09-06 and exhausted its restart allowance. The learner cleanly drained pending rows, wrote a final report and `latest.pt` at exactly 15,694,038 accepted decisions, and exited without an exception. The checkpoint SHA-256 is `419e4bfea65daee702d2dfca9dca43ef8fa978d664de266c38cb9a7ca44dd502`; all 3,401,455 generated expert targets had been consumed, no expert rows were stale or evicted, and all 742 optimizer proposals were accepted without a retry. The same configuration resumes from this checkpoint to 19,999,958, the largest target below 20,000,060 compatible with 512 environments. A final 102-environment sampling step will then reach the exact requested total.
...
The no-search control completed exactly 3,000,000 decisions. Trial 2 later exhausted the old whole-process watchdog after two genuine MCTS wedges; its next sampler generations imported the concurrently installed per-search-timeout code into a namespace created before that option existed and exited immediately. The intact checkpoint is 1,843,064. Its clean continuation is `target/full-combat-v70-a0-b4-3m-f05-s256-v8-q02-r1`, which inherits only parent reports through that checkpoint and uses the new watchdog-derived 60-second per-search bound. This changes only pathological searches that would otherwise be killed and discarded by the 120-second process watchdog.
