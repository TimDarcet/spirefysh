...
Conclusion: The patch is semantically clean. A custom Metal scatter-sum path was rejected because it matched native `index_add` throughput. Batch `4,096` is the candidate throughput configuration.

Next: Run two canonical 32,768-decision windows with four samplers, 512 environments, BF16, and batch `4,096` from a release Rust extension. Require at least `1,000` second-window end-to-end decisions/s, `99%` utilization, and no freshness or trust-region regression.
