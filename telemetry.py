...
import sts2_sim
from model import *
from trajectories import TrajectorySource, trajectory_page
_EVENT_FD = None
_EVENT_ROLE = None
...
class MetricsProjector:
    ...
    def fold_batch(self, event):
        ...
        policy_trained = event.get("policy_trained_rows", 0)
        trained = max(policy_trained, event.get("critic_trained_rows", 0))
        window["_trained"] += policy_trained
        self.decisions_trained += trained
        ...
        if commit and commit != "none" and duration > 0:
            window["_optimizer_steps"].append({
                ...
                "decisions_trained": self.decisions_trained,
        ...
        for outcome in ("accepted", "rejected"):
            ...
            for name in ("log_ratio", "trigger_log_ratio"):
                values = source.get(name)
                if not values:
                    continue
                count = values["count"]
                target[f"{name}_count"] = target.get(f"{name}_count", 0) + count
                target[f"{name}_sum"] = target.get(f"{name}_sum", 0.) \
                    + values["mean"] * count
                target[f"{name}_abs_sum"] = target.get(f"{name}_abs_sum", 0.) \
                    + values["abs_mean"] * count
                for metric in ("min", "max", "abs_max"):
                    value = values[metric]
                    function = min if metric == "min" else max
                    target[f"{name}_{metric}"] = function(
                        value, target.get(f"{name}_{metric}", value),
                    )
    ...
    def fold_packet(self, event, event_end):
        ...
        for index, episode in enumerate(event.get("episodes", ())):
            ...
            window["_boss_entries"] += int(episode["won"] or floor >= 51)
    ...
    def render(self, window):
        ...
        boss_entries = window["_boss_entries"]
        ...
        metrics |= {
            ...
            "win_rate_interval": wilson(wins, episodes),
            "boss_entries": boss_entries,
            "boss_entry_rate": boss_entries / max(1, episodes),
            "boss_entry_rate_interval": wilson(boss_entries, episodes),
            "boss_conversion": wins / max(1, boss_entries),
            "boss_conversion_interval": wilson(wins, boss_entries),
        ...
        for outcome, summary in window.get("_trajectory_outcomes", {}).items():
            ...
            for name in ("log_ratio", "trigger_log_ratio"):
                count = summary.get(f"{name}_count", 0)
                if not count:
                    continue
                ratio_prefix = f"{outcome}_{'trigger_' if name.startswith('trigger') else ''}"
                metrics[ratio_prefix + "log_ratio_rows"] = count
                metrics[ratio_prefix + "log_ratio_mean"] = summary[f"{name}_sum"] / count
                metrics[ratio_prefix + "abs_log_ratio_mean"] = \
                    summary[f"{name}_abs_sum"] / count
                metrics[ratio_prefix + "log_ratio_min"] = summary[f"{name}_min"]
                metrics[ratio_prefix + "log_ratio_max"] = summary[f"{name}_max"]
                metrics[ratio_prefix + "abs_log_ratio_max"] = summary[f"{name}_abs_max"]
...
def dashboard(target):
    ...
    for path in target.rglob("run.json"):
        ...
        for version, version_reports in version_groups.items():
            ...
            runs[key] = {
                "version": version, "label": name, "manifest": version_manifest,
...
body{font:14px system-ui;margin:24px;background:#101319;color:#e8ecf2}h1,h2{margin-bottom:6px}.controls,.legend{display:flex;align-items:center;gap:12px;flex-wrap:wrap}.controls{margin-bottom:12px}.legend{margin-bottom:22px}.legend-item{display:flex;align-items:center;gap:6px}.legend-line{width:24px;border-top:3px solid}.legend-dot{width:9px;height:9px;border-radius:50%}.metric-picker{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px;margin-top:12px}.metric-group{border:1px solid #303a49;border-radius:6px;padding:8px}.metric-group legend{font-weight:600}.metric-group label{display:block;padding:3px}summary{cursor:pointer;font-size:18px;font-weight:600}select,input{padding:7px;background:#202938;color:#e8ecf2;border:1px solid #526176;border-radius:5px}.charts{display:grid;grid-template-columns:repeat(auto-fit,minmax(440px,1fr));gap:14px}.panel{margin:22px 0;padding:16px;background:#171d28;border:1px solid #303a49;border-radius:10px}.charts .panel{margin:0}.plot{height:340px;min-width:0}.note{color:#9aa8bb;margin:0}table{border-collapse:collapse;width:100%}th,td{padding:7px;border-bottom:1px solid #303a49;text-align:left}.yes{color:#75db91}.no{color:#ee7b7b}
</style><p><a href=/trajectories.html>Trajectories</a></p><h1 id=title>Spirefysh dashboard</h1><div class=controls><label>Lineage <select id=lineage></select></label><label>Branch <select id=version></select></label><label>Compare <select id=compare></select></label><label>X axis <select id=xaxis><option value=updates>Optimizer updates</option><option value=decisions selected>Decisions trained on</option><option value=time>Active training time</option></select></label><label><input id=smooth type=checkbox checked> EMA</label><label>EMA α <input id=ema type=number min=.01 max=1 step=.01 value=.2></label><label>Subsample <input id=subsample type=number min=1 step=1 value=3></label><span>Auto-refresh 2s</span><span id=stage></span><span id=status></span></div><div id=legend class=legend></div><div class=charts><section class=panel><h2>Committed optimizer updates/s</h2><p class=note>One point per committed update</p><div id=optimizer-rate class=plot></div></section><section class=panel><h2>Critic rows processed/s</h2><p class=note>One point per committed update</p><div id=row-rate class=plot></div></section><section class=panel><h2>Run-ending floor</h2><div id=floor class=plot></div></section><section class=panel><h2>Window win rate</h2><div id=wins class=plot></div></section><section class=panel><h2>Boss-entry rate</h2><div id=boss-entry class=plot></div></section><section class=panel><h2>Post-update approximate policy KL</h2><div id=kl class=plot></div></section><section class=panel><h2>PPO clipped-row fraction</h2><div id=clip class=plot></div></section><section class=panel><h2>Training-row policy lag, p95 (revisions)</h2><div id=lag class=plot></div></section><section class=panel><h2>Training-buffer rows</h2><div id=dataset class=plot></div></section></div><details class=panel open><summary>Metrics</summary><div id=metrics class=metric-picker></div></details><section class=panel><h2>Data pipeline counters</h2><div id=rows></div></section><section class=panel><h2>Promotion</h2><div id=promotion></div></section><script>const versions=""" + data + r""",lineageSelect=document.querySelector('#lineage'),versionSelect=document.querySelector('#version'),compareSelect=document.querySelector('#compare'),xaxis=document.querySelector('#xaxis'),smooth=document.querySelector('#smooth'),ema=document.querySelector('#ema'),subsample=document.querySelector('#subsample'),metricPicker=document.querySelector('#metrics'),charts=document.querySelector('.charts');
const characterNames=['Ironclad','Defect','Silent','Regent','Necrobinder'],characterColors=['#ef4444','#38bdf8','#22c55e','#f59e0b','#a78bfa'],runColors=['#6fb1ff','#f472b6'],recency=Object.keys(versions),latest=recency.at(-1)||'',names=[...recency].sort((left,right)=>versions[left].label.localeCompare(versions[right].label,undefined,{numeric:true,sensitivity:'base'})),lineages=[...new Set(names.map(name=>versions[name].lineage_id||name))].sort((left,right)=>versions[names.find(name=>(versions[name].lineage_id||name)===left)].label.localeCompare(versions[names.find(name=>(versions[name].lineage_id||name)===right)].label,undefined,{numeric:true,sensitivity:'base'})),config={responsive:true,displaylogo:false},refreshKey='spirefysh-dashboard',saved=(()=>{try{return JSON.parse(sessionStorage.getItem(refreshKey))}catch{return null}})();let timeOrigin=0,sharedRange,syncingAxes=false,uiRevision='';const initial=saved?.followLatest?latest:names.includes(saved?.version)?saved.version:latest;lineageSelect.innerHTML=lineages.map(id=>`<option value="${id}">${versions[names.find(name=>(versions[name].lineage_id||name)===id)].label}</option>`).join('');lineageSelect.value=versions[initial]?.lineage_id||initial;function showBranches(preferred){const branches=names.filter(name=>(versions[name].lineage_id||name)===lineageSelect.value),latestBranch=recency.filter(name=>branches.includes(name)).at(-1)||'';versionSelect.innerHTML=branches.map(name=>`<option value="${name}">${versions[name].label}</option>`).join('');versionSelect.value=branches.includes(preferred)?preferred:latestBranch}showBranches(initial);compareSelect.innerHTML='<option value="">None</option>'+names.map(name=>`<option value="${name}">${versions[name].label}</option>`).join('');compareSelect.value=names.includes(saved?.compare)?saved.compare:'';if(saved?.xaxis)xaxis.value=saved.xaxis==='iteration'?'updates':saved.xaxis;if(typeof saved?.smooth==='boolean')smooth.checked=saved.smooth;if(saved?.ema)ema.value=saved.ema;if(saved?.subsample)subsample.value=saved.subsample;
const staticMetrics={optimizer_steps_per_second:'optimizer-rate',used_rows_per_second:'row-rate',floor_bands:'floor',win_rate:'wins',boss_entry_rate:'boss-entry',post_kl:'kl',clip_fraction:'clip',policy_lag_p95:'lag',dataset_rows:'dataset'},metricTitles={advantage_mean:'Raw advantage mean',advantage_stddev:'Raw advantage standard deviation',optimizer_steps_per_second:'Committed optimizer updates/s',used_rows_per_second:'Critic rows processed/s',floor_bands:'Run-ending floor',win_rate:'Window win rate',boss_entry_rate:'Boss-entry rate',boss_conversion:'Boss-entry conversion rate',clip_fraction:'PPO clipped-row fraction',pre_kl:'Pre-update approximate policy KL',post_kl:'Post-update approximate policy KL',entropy:'Policy entropy (nats)',entropy_weight:'Entropy loss weight',policy_loss:'PPO policy loss',critic_loss:'Weighted critic MSE',critic_explained_reward_variance:'Critic explained variance',critic_floor_conditioned_explained_reward_variance:'Within-floor critic explained variance',critic_expected:'Mean predicted return',critic_win_probability:'Critic-implied win score',total_seconds:'Optimizer update time (seconds)',unpack_seconds:'Batch unpack time (seconds)',forward_seconds:'Forward-pass time (seconds)',backward_seconds:'Backward-pass time (seconds)',policy_lag_mean:'Training-row policy lag, mean (revisions)',policy_lag_p95:'Training-row policy lag, p95 (revisions)',policy_lag_max:'Training-row policy lag, maximum (revisions)',gradient_norm:'Pre-clip global gradient norm',gradient_clipped:'Gradient clipping rate',dataset_rows:'Training-buffer rows',dataset_peak:'Peak training-buffer rows',row_utilization:'Attempted-row utilization',policy_row_utilization:'Attempted-row policy utilization',actor_decisions_per_second:'Actor decisions/s',learner_decisions_per_second:'Policy rows processed/s',decisions_per_second:'End-to-end decisions/s',sample_queue_delay_mean:'Sample queue delay (seconds)',sample_queue_depth:'Sample queue depth',sample_queue_capacity:'Sample queue capacity',accelerator_allocated_bytes:'Accelerator memory allocated (bytes)',accelerator_driver_allocated_bytes:'Accelerator driver memory (bytes)',card_cache_hit_rate:'Card-cache hit rate',graph_cache_hit_rate:'Graph-cache hit rate',trajectory_completion_seconds_mean:'Run completion time, mean (seconds)',trajectory_completion_seconds_max:'Run completion time, maximum (seconds)',trajectory_length_mean:'Run length, mean (decisions)',trajectory_length_max:'Run length, maximum (decisions)',trajectory_policy_span_mean:'Run policy span, mean (revisions)',trajectory_policy_span_max:'Run policy span, maximum (revisions)',trajectory_arrival_lag_mean:'Run arrival lag, mean (revisions)',trajectory_arrival_lag_max:'Run arrival lag, maximum (revisions)',winning_collection_drift:'Winning-replay policy drift (mean |Δ log p|)',expert_grad_cosine:'Expert–PPO policy-gradient cosine similarity',ppo_head_grad:'PPO policy-head gradient norm',expert_head_grad:'Expert policy-head gradient norm',mcts_root_fraction:'MCTS turn coverage',mcts_leaf_batch_mean:'MCTS inference batch size'},defaults=['floor_bands','win_rate','boss_entry_rate','critic_explained_reward_variance','critic_floor_conditioned_explained_reward_variance','post_kl','clip_fraction','policy_lag_p95','dataset_rows','optimizer_steps_per_second','used_rows_per_second'],hiddenMetrics=new Set(['iteration','steps','updates','weights_revision','policy_version','sampler_policy_revision','sampler_version','decisions_trained','seconds','mean_advantage','kl','observed_kl','observed_clip_fraction','accepted_decisions','dataset_seen','dataset_trained','discarded_steps','card_hits','card_misses','graph_hits','graph_misses','card_hit','card_miss','graph_hit','graph_miss']),aliases={advantage_mean:['mean_advantage'],post_kl:['kl'],clip_fraction:['observed_clip_fraction'],accelerator_allocated_bytes:['mps_allocated_bytes'],accelerator_driver_allocated_bytes:['mps_driver_allocated_bytes'],update_attempts:['policy_update_attempts'],policy_commits:['policy_updates_accepted']},allMetricKeys=[...new Set(defaults.concat(Object.keys(aliases),Object.values(versions).flatMap(run=>run.reports).flatMap(report=>Object.entries(report.metrics).filter(([,value])=>typeof value==='number'&&Number.isFinite(value)).map(([key])=>key))))].filter(key=>!hiddenMetrics.has(key)).sort(),metricIds=Object.fromEntries(allMetricKeys.filter(key=>!staticMetrics[key]).map((key,index)=>[key,`metric-${index}`]));let selectedMetrics=new Set(saved?.metrics?.filter(key=>allMetricKeys.includes(key))??defaults);function metricLabel(key){if(metricTitles[key])return metricTitles[key];const acronyms={kl:'KL',mcts:'MCTS',ppo:'PPO',p95:'p95',ema:'EMA',fifo:'FIFO'},words=key.split('_').map(word=>acronyms[word]||word);return words.map((word,index)=>index?word:word[0].toUpperCase()+word.slice(1)).join(' ')}function metricValue(report,key){for(const candidate of [key,...(aliases[key]||[])]){const value=Number(report.metrics[candidate]);if(Number.isFinite(value))return value}}function metricGroup(key){if(key==='floor_bands'||['win_rate','boss_entry_rate','boss_conversion','episodes','step_caps','combat_caps','empty_actions'].includes(key))return'Outcomes';if(key.startsWith('mcts_'))return'MCTS';if(key.startsWith('expert_')||key==='ppo_head_grad')return'Expert learning';if(key.startsWith('winning_'))return'Winning replay';if(key.startsWith('dataset_')||key.startsWith('accepted_')||key.startsWith('rejected_')||key.includes('trajectory_')||key.includes('trained_rows')||key.includes('utilization')||key.endsWith('_commits')||key.endsWith('_updates'))return'Data pipeline';if(key.includes('seconds')||key.includes('queue_')||key.includes('cache_')||key.includes('accelerator_')||key.includes('watchdog_'))return'Performance';return'Optimization'}function featureActive(run,key){const reports=run.reports;if(key.startsWith('mcts_'))return reports.some(report=>Number(report.metrics.mcts_roots)||Number(report.metrics.mcts_simulations));if(key.startsWith('expert_')||key==='ppo_head_grad')return reports.some(report=>Number(report.metrics.expert_rows)||Number(report.metrics.expert_rows_used)||Number(report.metrics.expert_rows_collected));if(key.startsWith('winning_'))return reports.some(report=>Number(report.metrics.winning_admitted)||Number(report.metrics.winning_added)||Number(report.metrics.winning_episodes)||Number(report.metrics.winning_replayed)||Number(report.metrics.winning_reservoir));if(key.startsWith('card_cache_'))return reports.some(report=>Number(report.metrics.card_hits??report.metrics.card_hit)||Number(report.metrics.card_misses??report.metrics.card_miss));if(key.startsWith('graph_cache_'))return reports.some(report=>Number(report.metrics.graph_hits??report.metrics.graph_hit)||Number(report.metrics.graph_misses??report.metrics.graph_miss));if(key.startsWith('accelerator_'))return reports.some(report=>metricValue(report,key));return true}function metricPresent(run,key){if(key==='floor_bands')return run.reports.some(report=>report.metrics.floor_bands||report.metrics.trajectory_floors?.length);return run.reports.some(report=>metricValue(report,key)!==undefined)}function visibleRuns(){return[versions[versionSelect.value],compareSelect.value&&versions[compareSelect.value]].filter(Boolean)}function availableMetricKeys(){const runs=visibleRuns();return allMetricKeys.filter(key=>runs.some(run=>metricPresent(run,key)&&featureActive(run,key)))}function renderMetricPicker(){const groups={};for(const key of availableMetricKeys())(groups[metricGroup(key)]??=[]).push(key);metricPicker.innerHTML=Object.entries(groups).map(([group,keys])=>`<fieldset class=metric-group><legend>${group}</legend>${keys.map(key=>`<label><input type=checkbox value="${key}"${selectedMetrics.has(key)?' checked':''}> ${metricLabel(key)}</label>`).join('')}</fieldset>`).join('')}function updateMetricPanels(){renderMetricPicker();const available=new Set(availableMetricKeys());for(const [key,id] of Object.entries(staticMetrics))document.querySelector(`#${id}`).parentElement.hidden=!available.has(key)||!selectedMetrics.has(key);for(const [key,id] of Object.entries(metricIds)){let node=document.querySelector(`#${id}`);if(available.has(key)&&selectedMetrics.has(key)&&!node){charts.insertAdjacentHTML('beforeend',`<section class=panel><h2>${metricLabel(key)}</h2><div id="${id}" class=plot></div></section>`);node=document.querySelector(`#${id}`)}if(node)node.parentElement.hidden=!available.has(key)||!selectedMetrics.has(key)}}metricPicker.onchange=()=>{for(const key of availableMetricKeys())selectedMetrics.delete(key);for(const input of metricPicker.querySelectorAll('input:checked'))selectedMetrics.add(input.value);showVersion(false)};
function rateSeries(history,key,intervalKey){return history.map(report=>{const y=metricValue(report,key),interval=report.metrics[intervalKey];return{x:x(report),y,low:Number(interval?.[0]),high:Number(interval?.[1])}}).filter(point=>Number.isFinite(point.y))}
function monotonic(history){let step=-Infinity;return history.filter(row=>row.step>step&&(step=row.step,true))}function updateSteps(history){let offset=0,last=0,session;for(const report of history){const updates=Number(report.metrics.weights_revision??report.metrics.updates)||0;if(session!==undefined&&(report._session!==session||updates<last)){offset+=last;last=0}report._updates=offset+updates;last=Math.max(last,updates);session=report._session}}function trained(report){return Number(report.metrics.decisions_trained??report.metrics.dataset_trained??report.step)}function x(report){return xaxis.value==='time'?(Number.isFinite(Number(report.metrics.seconds))?report.metrics.seconds/60:Number.isFinite(Number(report._written))?(report._written-timeOrigin)/60:0):xaxis.value==='decisions'?trained(report):report._updates}function optimizerX(step){return xaxis.value==='time'?step.seconds/60:xaxis.value==='decisions'?Number(step.decisions_trained??step.step):step._updates}function series(history,key,fallback){return history.map(report=>({x:x(report),y:metricValue(report,key)??Number(report.metrics[fallback])})).filter(point=>Number.isFinite(point.y))}function optimizerSeries(steps,history,key){return steps.length?steps.map(step=>({x:optimizerX(step),y:Number(step[key])})):series(history.filter(report=>(report.metrics.update_attempts??1)>0),key)}function optimizerSteps(run,reports){if(!run.optimizer_steps)run.optimizer_steps=reports.flatMap(report=>{const offset=report._updates-Number(report.metrics.weights_revision??report.metrics.updates);return(report.metrics.optimizer_steps||[]).map(step=>({...step,_updates:offset+step.weights_revision}))});return run.optimizer_steps}function speedMetrics(history){for(const report of history){const metrics=report.metrics,seconds=Number(metrics.total_seconds)*Number(metrics.update_attempts);if(!Number.isFinite(Number(metrics.optimizer_steps_per_second))&&seconds>0)metrics.optimizer_steps_per_second=Number(metrics.weight_commits)/seconds;if(!Number.isFinite(Number(metrics.used_rows_per_second)))metrics.used_rows_per_second=seconds>0?Number(metrics.critic_trained_rows)/seconds:Number(metrics.learner_decisions_per_second)}}function prepareRun(run){const all=monotonic(run.reports);updateSteps(all);const reports=all.filter(row=>!row._open),steps=optimizerSteps(run,all),last=all.at(-1),revision=Number(last?.metrics.weights_revision??last?.metrics.updates)||0,timed=all.find(row=>Number.isFinite(Number(row._written))&&Number.isFinite(Number(row.metrics.seconds)));run._updateOffset=(last?last._updates:0)-revision;run._timeOrigin=timed?timed._written-timed.metrics.seconds:0;speedMetrics(reports);return{run,reports,steps}}function rangeOf(values){let low=Infinity,high=-Infinity;for(const value of values)if(Number.isFinite(value)){low=Math.min(low,value);high=Math.max(high,value)}if(low===Infinity)return;const padding=(high-low||Math.abs(high)*.02||1)*.02;return[low-padding,high+padding]}function setFullRange(data){sharedRange=rangeOf(data.flatMap(({run,reports,steps})=>{timeOrigin=run._timeOrigin;return reports.map(x).concat(steps.map(optimizerX))}))}function clipComparison(comparison,primary){timeOrigin=primary.run._timeOrigin;const end=Math.max(...primary.reports.map(x).concat(primary.steps.map(optimizerX)).filter(Number.isFinite));timeOrigin=comparison.run._timeOrigin;return{...comparison,end,reports:comparison.reports.filter(report=>x(report)<=end),steps:comparison.steps.filter(step=>optimizerX(step)<=end)}}
...
function rowSummary(history){const items=[['Collection','dataset_admitted','Rows admitted to training buffer'],['Collection','dataset_rollout_dropped','Unusable rollout decisions'],['Collection','dataset_budget_dropped','Rows beyond decision budget'],['Collection','dataset_forced_dropped','Forced-action rows excluded'],['Buffer','dataset_capacity_dropped','Rows evicted for capacity'],['Buffer','dataset_stale_dropped','Stale rows evicted'],['Training','dataset_attempted','Rows selected for training'],['Training','critic_trained_rows','Rows used by critic'],['Training','policy_trained_rows','Rows used by policy'],['Training','dataset_ratio_dropped','Rows rejected by behavior ratio'],['Training','dataset_kl_dropped','Policy rows rejected before update'],['Training','dataset_post_kl_dropped','Policy rows rejected after trial'],['Retirement','dataset_retired','Rows retired after use']],projected=history.filter(row=>'update_attempts'in row.metrics),rows=projected.length?projected:history.slice(-1),latest=rows.at(-1)?.metrics||{},number=value=>Number.isFinite(value)?value.toLocaleString():'—',body=items.map(([stage,key,label])=>{const values=rows.map(row=>Number(row.metrics[key])).filter(Number.isFinite);return `<tr><td>${stage}</td><td>${label}</td><td>${number(Number(latest[key]))}</td><td>${number(values.length?values.reduce((sum,value)=>sum+value,0):NaN)}</td></tr>`}).join('');return `<table><thead><tr><th>Stage</th><th>Counter</th><th>Latest window</th><th>Run history total</th></tr></thead><tbody>${body}</tbody></table>`}
function saveDashboardState(){try{sessionStorage.setItem(refreshKey,JSON.stringify({version:versionSelect.value,compare:compareSelect.value,metrics:[...selectedMetrics],followLatest:versionSelect.value===latest,xaxis:xaxis.value,smooth:smooth.checked,ema:ema.value,subsample:subsample.value,scroll:[scrollX,scrollY]}))}catch{}}function restoreDashboardState(){if(saved?.scroll)scrollTo(...saved.scroll)}
function showStatus(run){const age=Date.now()/1000-(run.last_event_time||0),timeout=run.manifest.sessions?.at(-1)?.training?.sampler_timeout||120,current=run.reports.at(-1)?.stage||{},stage=[Number.isFinite(Number(current.ascension))?`curriculum A${current.ascension}`:'',Number(current.bonus)?`training bonus +${current.bonus}`:''].filter(Boolean).join(' · ');document.querySelector('#stage').textContent=stage;document.querySelector('#status').textContent=run.status==='running'&&age>timeout?'stalled':run.status||''}function showLegend(primary,comparison){const runs=[primary,comparison].filter(Boolean),runItems=runs.map((item,index)=>`<span class=legend-item><i class=legend-line style="border-color:${runColors[index]}"></i>${item.run.label}</span>`).join(''),characters=characterNames.map((name,index)=>`<span class=legend-item><i class=legend-dot style="background:${characterColors[index]}"></i>${name}</span>`).join('');document.querySelector('#legend').innerHTML=`${runItems}<span class=note>${smooth.checked?'thin raw · bold EMA':'raw'} · run-ending floors:</span>${characters}`}const percentMetrics=new Set(['boss_conversion','gradient_clipped','row_utilization','policy_row_utilization','card_cache_hit_rate','graph_cache_hit_rate','mcts_root_fraction','mcts_simulate_fraction','mcts_encode_fraction','mcts_inference_fraction','mcts_backup_fraction','mcts_rollout_fraction','winning_replay_fraction','discarded_step_fraction','trajectory_stale_step_fraction']);function dynamicPlots(reports,run,label,index=0){for(const [key,id] of Object.entries(metricIds))if(selectedMetrics.has(key)){const points=series(reports,key);index?addPlot(id,points,label):plot(id,points,{history:reports,run,label,percent:percentMetrics.has(key),range:percentMetrics.has(key)?[0,1]:undefined})}}function addComparison({run,reports,steps}){timeOrigin=run._timeOrigin;const label=run.label;addPlot('optimizer-rate',optimizerSeries(steps,reports,'optimizer_steps_per_second'),label);addPlot('row-rate',optimizerSeries(steps,reports,'used_rows_per_second'),label);addFloor(reports,label);addPlot('wins',rateSeries(reports,'win_rate','win_rate_interval'),label);addPlot('boss-entry',rateSeries(reports,'boss_entry_rate','boss_entry_rate_interval'),label);addPlot('kl',series(reports,'post_kl'),label);addPlot('clip',series(reports,'clip_fraction'),label);addPlot('lag',series(reports,'policy_lag_p95'),label);addPlot('dataset',series(reports,'dataset_rows'),label);dynamicPlots(reports,run,label,1)}function showVersion(resetRange=true){if(compareSelect.value===versionSelect.value)compareSelect.value='';for(const option of compareSelect.options)option.disabled=option.value===versionSelect.value;const primary=prepareRun(versions[versionSelect.value]),{run,reports,steps}=primary,selectedComparison=compareSelect.value&&prepareRun(versions[compareSelect.value]),comparison=selectedComparison&&clipComparison(selectedComparison,primary),label=comparison?run.label:'';updateMetricPanels();uiRevision=`${versionSelect.value}:${compareSelect.value}:${xaxis.value}`;if(resetRange)setFullRange([primary]);timeOrigin=run._timeOrigin;document.querySelector('#title').textContent=`Spirefysh · ${run.label}${comparison?` vs ${comparison.run.label}`:''}`;showStatus(run);showLegend(primary,comparison);plot('optimizer-rate',optimizerSeries(steps,reports,'optimizer_steps_per_second'),{tozero:true,history:reports,run,label});plot('row-rate',optimizerSeries(steps,reports,'used_rows_per_second'),{tozero:true,history:reports,run,label});floorPlot(reports,run,label);plot('wins',rateSeries(reports,'win_rate','win_rate_interval'),{range:[0,1],percent:true,history:reports,run,label});plot('boss-entry',rateSeries(reports,'boss_entry_rate','boss_entry_rate_interval'),{range:[0,1],percent:true,history:reports,run,label});plot('kl',series(reports,'post_kl'),{tozero:true,history:reports,run,label});plot('clip',series(reports,'clip_fraction'),{range:[0,1],percent:true,history:reports,run,label});plot('lag',series(reports,'policy_lag_p95'),{tozero:true,history:reports,run,label});plot('dataset',series(reports,'dataset_rows'),{tozero:true,history:reports,run,label});dynamicPlots(reports,run,label);if(comparison)addComparison(comparison);timeOrigin=run._timeOrigin;document.querySelector('#rows').innerHTML=rowSummary(reports);document.querySelector('#promotion').innerHTML=promotionSummary(run.promotions.at(-1));bindAxes()}
function extendFloor(history,index=0){const completed=history.filter(report=>Number(report.metrics.episodes)>0||(report.metrics.trajectory_floors?.length??0)>0);if(!completed.length)return;const node=document.querySelector('#floor'),state=node._floor[index],bands=[['min','max'],['p01','p99'],['p05','p95'],['p10','p90'],['p25','p75']],xs=[],ys=[];for(const [band,[low,high]] of bands.entries()){let rows=completed.map(report=>({x:x(report),low:report.metrics.floor_bands?.[low],high:report.metrics.floor_bands?.[high]})).filter(row=>Number.isFinite(row.low)&&Number.isFinite(row.high)),visible=sampled(rows,state.bands[band]);state.bands[band]+=rows.length;xs.push(visible.map(row=>row.x),visible.map(row=>row.x));ys.push(visible.map(row=>row.low),visible.map(row=>row.high))}if(xs.some(values=>values.length))Plotly.extendTraces(node,{x:xs,y:ys},bands.flatMap((_,band)=>[state.start+2*band,state.start+2*band+1]));let points=trajectoryPoints(completed),visible=sampled(points,state.points);state.points+=points.length;if(visible.length)Plotly.extendTraces(node,{x:[visible.map(point=>point.x)],y:[visible.map(point=>point.y)],customdata:[visible.map(point=>[characterNames[point.character],point.updates,point.decisions])],'marker.color':[visible.map(point=>characterColors[point.character])]},[state.start+10]);const means=completed.map(report=>({x:x(report),y:report.metrics.floor_bands?.mean??report.metrics.floor_mean})).filter(point=>Number.isFinite(point.y)),offset=state.means,line=means.map(point=>({x:point.x,y:state.ema=state.ema===undefined?point.y:Math.max(.01,Math.min(1,Number(ema.value)||.2))*point.y+(1-Math.max(.01,Math.min(1,Number(ema.value)||.2)))*state.ema})),shown=sampled(means,offset),update={x:[shown.map(point=>point.x)],y:[shown.map(point=>point.y)]},traces=[state.start+11];state.means+=means.length;if(state.smooth!==null){visible=sampled(line,offset);update.x.push(visible.map(point=>point.x));update.y.push(visible.map(point=>point.y));traces.push(state.smooth)}if(update.x.some(values=>values.length))Plotly.extendTraces(node,update,traces)}function extendStage(id,reports,key,index){let points=reports.filter(row=>Number.isFinite(Number(row.stage?.[key]))).map(row=>({x:x(row),y:row.stage[key]}));if(!points.length)return;const node=document.querySelector(`#${id}`),state=node._stage[index],visible=sampled(points,state.seen);state.seen+=points.length;if(visible.length)Plotly.extendTraces(node,{x:[visible.map(point=>point.x)],y:[visible.map(point=>point.y)]},[state.trace])}
function appendData(run,reports,steps){run.reports=run.reports.filter(row=>!row._open).concat(reports);const all=monotonic(run.reports);updateSteps(all);const last=all.at(-1),revision=Number(last?.metrics.weights_revision??last?.metrics.updates)||0;run._updateOffset=(last?last._updates:0)-revision;speedMetrics(reports);for(const step of steps)step._updates=run._updateOffset+step.weights_revision;run.optimizer_steps.push(...steps);return all}function appendReports(run,reports,steps,index=0){const all=appendData(run,reports,steps);extendPlot('optimizer-rate',optimizerSeries(steps,[],'optimizer_steps_per_second'),index);extendPlot('row-rate',optimizerSeries(steps,[],'used_rows_per_second'),index);if(selectedMetrics.has('floor_bands'))extendFloor(reports,index);extendPlot('wins',rateSeries(reports,'win_rate','win_rate_interval'),index);extendPlot('boss-entry',rateSeries(reports,'boss_entry_rate','boss_entry_rate_interval'),index);extendPlot('clip',series(reports,'clip_fraction'),index);extendPlot('kl',series(reports,'post_kl'),index);extendPlot('lag',series(reports,'policy_lag_p95'),index);extendPlot('dataset',series(reports,'dataset_rows'),index);for(const [key,id] of Object.entries(metricIds))if(selectedMetrics.has(key))extendPlot(id,series(reports,key),index);if(index===0){const history=all.filter(row=>!row._open);for(const plot of visiblePlots())Plotly.relayout(plot,{shapes:stageLines(history,run)});document.querySelector('#rows').innerHTML=rowSummary(history);document.querySelector('#promotion').innerHTML=promotionSummary(run.promotions.at(-1));showStatus(run)}}
...
lineageSelect.onchange=()=>{showBranches();showVersion()};versionSelect.onchange=()=>showVersion();compareSelect.onchange=()=>showVersion();xaxis.onchange=()=>showVersion();smooth.onchange=()=>showVersion(false);ema.oninput=()=>showVersion(false);subsample.oninput=()=>showVersion(false);showVersion();bindAxes();setTimeout(restoreDashboardState,100);window.addEventListener('beforeunload',saveDashboardState);setInterval(refreshLive,2000)</script>"""
    content = content.replace(
        "<details class=panel open>", "<details class=panel>",
    ).replace(
        "options.yaxis.title='Terminal floor'", "options.yaxis.title='Run-ending floor'",
    ).replace(
...
def serve_dashboard(target, host, port):
    ...
    trajectories = TrajectorySource(target)
    ...
    class Handler(http.server.BaseHTTPRequestHandler):
        def do_GET(self):
            ...
            try:
                ...
                elif request.path == "/trajectories.html":
                    body, content_type = trajectory_page().encode(), "text/html"
                elif request.path == "/api/trajectories":
                    query = parse_qs(request.query)
                    value = trajectories.list(query["run"][0]) if "run" in query \
                        else trajectories.catalog()
                    body, content_type = json.dumps(value, separators=(",", ":")).encode(), \
                        "application/json"
                elif request.path == "/api/trajectory":
                    query = parse_qs(request.query)
                    value = trajectories.get(query["run"][0], query["id"][0])
                    body, content_type = json.dumps(value, separators=(",", ":")).encode(), \
                        "application/json"
        ...
        def do_POST(self):
            try:
                ...
                if path not in ("/api/trajectory", "/api/explanation"):
                    self.send_error(404); return
                ...
                if path == "/api/explanation":
                    value = trajectories.explain(
                        run, request["trajectory"], request["step"], request.get("case"),
                    )
                else:
                    value = trajectories.add(run, request["trace"]) if "trace" in request \
                        else trajectories.generate(run, request)
