...
struct simdgroup_event {
  ushort lane;
  METAL_FUNC simdgroup_event() thread : lane(0) {}
...
constant uint A_leading_dimension = 128;
constant uint B_leading_dimension = 128;
constant uint C_leading_dimension = 128;

// Whether to load the previous value of C, and add it to the accumulator.
constant bool load_previous_C = false;
...
constant ushort N_remainder = (N % 32 == 0)
  ? 32 : N % 32;
...
constant ushort N_shift = (N < N_group) ? 0 : 32 - N_remainder;
...
const device bfloat *A_src,
const device bfloat *B_src,
thread simdgroup_matrix_storage<bfloat> *A_sram,
thread simdgroup_matrix_storage<bfloat> *B_sram,
thread simdgroup_matrix_storage<float> *C_sram,
ushort k
) {
#pragma clang loop unroll(full)
for (ushort m = 0; m < 32; m += 8) {
  ushort2 origin(0, m);
...
for (ushort n = 0; n < 32; n += 8) {
  ushort2 origin(n, 0);
  auto B = get_sram(B_sram, 32, origin);
...
#pragma clang loop unroll(full)
  for (ushort n = 0; n < 32; n += 8) {
    ...
    auto B = get_sram(B_sram, 32, ushort2(n, 0));
    auto C = get_sram(C_sram, 32, ushort2(n, m));
    C->multiply(*A, *B);
  }
}
}
...
// One multiply-accumulate loop iteration, or 8 dot products.
METAL_FUNC void multiply_accumulate(
const threadgroup bfloat *A_src,
const threadgroup bfloat *B_src,
thread simdgroup_matrix_storage<bfloat> *A_sram,
thread simdgroup_matrix_storage<bfloat> *B_sram,
thread simdgroup_matrix_storage<float> *C_sram,
ushort k
...
for (ushort m = 0; m < 32; m += 8) {
  ushort2 origin(0, m);
  auto A = get_sram(A_sram, 8, origin);
  A->load(A_src, 8, ushort2(k, m), A_trans);
}
#pragma clang loop unroll(full)
for (ushort n = 0; n < 32; n += 8) {
  ushort2 origin(n, 0);
  auto B = get_sram(B_sram, 32, origin);
  B->load(B_src, 8, ushort2(n, k), B_trans);
}
#pragma clang loop unroll(full)
for (ushort m = 0; m < 32; m += 8) {
#pragma clang loop unroll(full)
...
kernel void linear_128_128(device bfloat *A [[buffer(0)]],
                 ...
                 ushort lane_id [[thread_index_in_simdgroup]])
{
  ...
  ushort M_remainder = (M % 32 == 0) ? 32 : M % 32;
  ushort M_shift = (M < M_group) ? 0 : 32 - M_remainder;
  ushort2 sid(sidx % 1, sidx / 1);
  ...
  if (M_offset + sid.y * 32 >= M ||
      N_offset + sid.x * 32 >= N) {
    return;
  }
  ushort2 offset_in_group(sid.x * 32 + morton_offset.x,
                          sid.y * 32 + morton_offset.y);
...
if ((M >= M_group) && (N >= N_group) && (load_previous_C ? (M_offset == gid.y * M_group) : true) && (load_previous_C ? (N_offset == gid.x * N_group) : true)) {
  // Fast path for matrices that qualify.
  uint2 C_offset(N_offset + offset_in_group.x,
                 M_offset + offset_in_group.y);
  auto C_dst = simdgroup_matrix_storage<bfloat>::apply_offset(
...
#pragma clang loop unroll(full)
    for (ushort n = 0; n < 32; n += 8) {
      ushort2 origin(n, m);
      auto C = get_sram(C_sram, 32, origin);
      C->load_bfloat(C_dst, C_leading_dimension, origin);
    }
  }
} else {
  // Slow path for when memory must be handled more carefully.
  auto C_block = (threadgroup bfloat*)(threadgroup_block);
  auto C_block_dst =
...
for (uint k = 0; k < (K - (K % K_group)); k += 8) {
  ...
  auto A_src = simdgroup_matrix_storage<bfloat>::apply_offset(
    A, A_leading_dimension, A_offset, A_trans);
  auto B_src = simdgroup_matrix_storage<bfloat>::apply_offset(
    B, B_leading_dimension, B_offset, B_trans);

  simdgroup_matrix_storage<bfloat> A_sram[
    4 * (8 / 8)];
  simdgroup_matrix_storage<bfloat> B_sram[
    (8 / 8) * 4];
...
for (uint k = (K - (K % K_group)); k < K; k += K_group) {
  auto A_block = (threadgroup bfloat*)(
    threadgroup_block);
  auto B_block = (threadgroup bfloat*)(
    threadgroup_block + 512);
  ...
  if (sidx == 0) {
    ...
    simdgroup_event events[2] = {lane_id, lane_id};
    events[0].async_copy(
      A_block, 8, A_tile_dst,
      A_src, A_leading_dimension, A_tile_src, A_trans);
    events[1].async_copy(
      B_block, 8, B_tile_dst,
  ...
  A_block_src = simdgroup_matrix_storage<bfloat>::apply_offset(
    A_block_src, 8, A_block_offset, A_trans);
  B_block_src = simdgroup_matrix_storage<bfloat>::apply_offset(
    B_block_src, 8, B_block_offset, B_trans);
...
#pragma clang loop unroll(full)
    for (ushort n = 0; n < 32; n += 8) {
      auto elements = get_sram(C_sram, 32, ushort2(n, m))->thread_elements();
      uint column = N_offset + n + morton_offset.x;
      (*elements)[0] += float(bias[min(column, N - 1)]);
      (*elements)[1] += float(bias[min(column + 1, N - 1)]);
    }
  }
...
#pragma clang loop unroll(full)
    for (ushort n = 0; n < 32; n += 8) {
      ushort2 origin(n, m);
      auto C = get_sram(C_sram, 32, origin);
      C->store_bfloat(C_dst, C_leading_dimension, origin);
    }
  }
} else {
  // Slow path for when memory must be handled more carefully.
...
#pragma clang loop unroll(full)
  ...
  if (sidx == 0) {
    ...
    if ((M_shift != 0) || (N_shift != 0)) {
      ...
      C_block = simdgroup_matrix_storage<bfloat>::apply_offset(
...
constant uint A_leading_dimension = 128;
constant uint B_leading_dimension = 128;
constant uint C_leading_dimension = 384;
...
kernel void linear_128_384(device bfloat *A [[buffer(0)]],
                 device bfloat *B [[buffer(1)]],
                 device bfloat *C [[buffer(2)]],
                 device const bfloat *bias [[buffer(3)]],
                 constant uint& M [[buffer(4)]],
...
kernel void linear_residual_128_128(device bfloat *A [[buffer(0)]],
                 device bfloat *B [[buffer(1)]],
...
#pragma clang loop unroll(full)
    for (ushort n = 0; n < 32; n += 8) {
      auto elements = get_sram(C_sram, 32, ushort2(n, m))->thread_elements();
      uint column = N_offset + n + morton_offset.x;
      (*elements)[0] = float(bfloat((*elements)[0] + float(bias[min(column, N - 1)])));
      (*elements)[1] = float(bfloat((*elements)[1] + float(bias[min(column + 1, N - 1)])));
      uint row = M_offset + m + morton_offset.y;
      (*elements)[0] += residual[row * N + column];
      (*elements)[1] += residual[row * N + column + 1];
    }
  }
